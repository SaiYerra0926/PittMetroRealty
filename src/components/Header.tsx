  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-slate-100/95 backdrop-blur-md shadow-lg border-b border-slate-200 safe-top">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
        {/* Main navigation */}
        <nav className="flex items-center justify-between py-1 sm:py-1.5 md:py-2 gap-2 sm:gap-2.5">
          {/* Logo */}
          ...
          {/* Desktop Navigation */}
          <div className="hidden lg:flex items-center gap-1.5 md:gap-2 lg:gap-2.5 xl:gap-3">
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`relative px-2 sm:px-2.5 md:px-3 py-1 font-medium text-xs sm:text-sm transition-all duration-300 hover:scale-105 group touch-target ${
                    isActive 
                      ? 'text-primary' 
          ...
          {/* Contact Info */}
          <div className="hidden xl:flex items-center gap-2.5 lg:gap-3">
            <div className="flex items-center gap-1.5 text-slate-700 hover:text-primary transition-all duration-300 cursor-pointer hover:scale-105 group touch-target">
            ...
            <div className="hidden 2xl:flex items-center gap-1.5 text-slate-700 hover:text-primary transition-all duration-300 cursor-pointer hover:scale-105 group touch-target">
              <div className="w-5 h-5 bg-blue-100 rounded-lg flex items-center justify-center group-hover:bg-blue-200 transition-all duration-300">
          ...
          {/* Mobile menu button */}
          <button
            className="xl:hidden p-1.5 sm:p-2 rounded-lg hover:bg-slate-100 transition-all duration-300 hover:scale-105 group touch-target min-w-[44px] min-h-[44px] flex items-center justify-center"
            onClick={() => setIsMenuOpen(!isMenuOpen)}
            aria-label="Toggle menu"
          >
            <div className="w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center">
              {isMenuOpen ? (
                <X className="h-4 w-4 sm:h-5 sm:w-5 text-gray-700 group-hover:text-primary transition-all duration-300" />
              ) : (
                <Menu className="h-4 w-4 sm:h-5 sm:w-5 text-gray-700 group-hover:text-primary transition-all duration-300" />
              )}
            </div>
          </button>
        </nav>
        ...
        {/* Mobile Navigation */}
        {isMenuOpen && (
          <div className="xl:hidden py-3 sm:py-4 border-t border-slate-200 bg-slate-100/98 backdrop-blur-md animate-slide-in-down safe-bottom max-h-[calc(100vh-80px)] overflow-y-auto">
              ...
              {navItems.map((item, index) => {
                ...
                return (
                  <button
                    key={item.label}
                    onClick={() => handleNavClick(item.href)}
                    className={`py-2.5 sm:py-3 px-4 text-sm sm:text-base text-left font-medium transition-all duration-300 rounded-lg hover:scale-105 touch-target min-h-[44px] ${
                      isActive 
