...
export const testConnection = async (retries = 3) => {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      console.error(`❌ Database connection attempt ${attempt} failed:`, error instanceof Error ? error.message : 'Unknown error');
      
      if (attempt === retries) {
        console.error('❌ All connection attempts failed. Please check:');
        console.error('   1. Database host is correct:', dbConfig.host);
        console.error('   2. Database credentials are correct');
        console.error('   3. Network/firewall allows connections to port', dbConfig.port);
        console.error('   4. Database server is running and accessible');
        return false;
      }
